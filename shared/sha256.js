/* SHA-256 —— 纯 JS，浏览器和 Node 共用。只用于写入侧的工作量证明，不用于加密。
   这份实现逐字节对齐 node:crypto（含 200 组随机二进制回归）。 */
...
export function sha256Bytes(input) {
  ...
  const total = Math.floor((len + 9 + 63) / 64) * 64;
  ...
  for (let off = 0; off < total; off += 64) {
    for (let i = 0; i < 16; i++) w[i] = view.getUint32(off + (i << 2), false);
    for (let i = 16; i < 64; i++) {
